...
namespace posh
{
    ...
    bool starts_with_any_of(const std::string &str, const std::string &substr)
	{
        for (char c : substr)
		{
            if (str.starts_with(std::to_string(c))) return true;
        }
    ...
    bool starts_with(const std::string &str, const CharType &chartype, int range)
	{
	    ...
	    for (char charx : str)
		{
            if (chartype == CT_ALPHA && !starts_with_any_of(std::to_string(charx), "abcdefghijklmnopqrstuvwxyzABCDEVGHIJKLMNOPQRSTUVWXYZ")) return false;
			
			else if (chartype == CT_MISC && !starts_with_any_of(std::to_string(charx), "~!@#$%^&*()_+`-={}[]|\\:;\"'<>,.?/")) return false;
			
			else if (chartype == CT_INT && !starts_with_any_of(std::to_string(charx), "0123456789")) return false;
			
			else if (chartype == CT_MATH && !(starts_with_any_of(std::to_string(charx), "0123456789") || starts_with_any_of(std::to_string(charx), "!%^*-+=/"))) return false;
			
			else if (chartype == CT_LOGIC && !(starts_with_any_of(std::to_string(charx), "0123456789") || starts_with_any_of(std::to_string(charx), "!&|"))) return false;

            if (single_char) return true;
    ...
    bool starts_with(const std::string &str, const double doubles[], const int &size)
	{
		...
		{
            if (str.starts_with(std::to_string(doubles[i]))) return true;
        }
    ...
    bool starts_with(const std::string &str, const float floats[], const int &size)
	{
        for (int i=0; i<size; i++)
		{
            if (str.starts_with(std::to_string(floats[i]))) return true;
        }
    ...
    bool starts_with(const std::string &str, const int ints[], const int &size)
	{
        for (int i=0; i<size; i++)
		{
            if (str.starts_with(std::to_string(ints[i]))) return true;
        }
    ...
    bool starts_with(const std::string &str, const char chars[], const int &size)
	{
        for (int i=0; i<size; i++)
		{
            if (str.starts_with(std::to_string(chars[i]))) return true;
        }
    ...
    std::vector<std::string> split_str(const std::string &str, const std::string &delim)
	{
	    ...
	    for (int i = 0; i < str.size(); i++)
		{
		    ...
		    for (int j = 0; j < delim.size(); j++) if (str[i + j] != delim[j]) flag = false;
    ...
    std::string vec_to_str(std::vector<std::string> &vec, const bool brackets)
	{
        std::string result;
		...
		if (brackets)
		{
            if (!vec.empty())
			{
                if (vec.size() > 1)
				{
					...
					{
                        if (i == vec.size() - 1) result += vec[i] + "}";

                        else if (i == 0) result += std::string("{") + vec[i] + ", ";
						
                        else result += vec[i] + ", ";
                    }
                } 
				
				else result += std::string("{") + vec[0] + "}";
            } 
    ...
    // print a vector
    void print_vec(std::vector<std::string> &vec, const bool brackets, const bool endl)
	{
        if (brackets)
		{
            if (!vec.empty())
